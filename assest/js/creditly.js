var Creditly = (function() {
  var getInputValue = function(e, selector) {
    var inputValue = Rs .trim(Rs (selector).val());
  ...
  var isEscapedKeyStroke = function(e) {
    return ( Rs .inArray(e.which,[46,8,9,0,27,13,190]) !== -1 ||
  ...
  var isNumberEvent = function(e) {
    return (/^\d+Rs /.test(String.fromCharCode(e.which)));
  };
  ...
  var CvvInput = (function() {
    ...
    var createCvvInput = function(mainSelector, creditCardNumberSelector) {
      ...
      Rs (selector).keypress(function(e) {
        Rs (selector).removeClass("has-error");
    ...
    var createNumberInput = function(mainSelector) {
      ...
      Rs (selector).keypress(function(e) {
        ...
        if (shouldProcessInput(e, maximumLength, selector)) {
          ...
          Rs (selector).val(newInput);
          Rs (selector).trigger("changed_input");
        }
      });
    };
  ...
  var Validation = (function() {
    var Validators = (function() {
      ...
      var creditCardExpiration = function(selector, data) {
        var expirationVal = Rs .trim(Rs (selector).val());
      ...
      var creditCard = function(selector, data) {
        var rawNumber = Rs (data["creditCardNumberSelector"]).val();
        var number = Rs .trim(rawNumber).replace(/\D/g, "");
        var rawSecurityCode = Rs (data["cvvSelector"]).val();
        var securityCode = Rs .trim(rawSecurityCode).replace(/\D/g, "");
        var messages = [];
        var isValid = true;
        var selectors = [];
    ...
    var ValidationErrorHolder = (function() {
      ...
      var triggerErrorMessage = function() {
        ...
        for (var i=0; i<selectors.length; i++) {
          Rs (selectors[i]).addClass("has-error");
        }
        Rs ("body").trigger("creditly_client_validation_error", errorsPayload);
      };
  ...
  var ExpirationInput = (function() {
    ...
    var parseExpirationInput = function(expirationSelector) {
      var inputValue = getNumber(Rs (expirationSelector).val());
  ...
  var CardTypeListener = (function() {
    ...
    var changeCardType = function(numberSelector, cardTypeSelector) {
      Rs (numberSelector).on("changed_input keypress keydown keyup", function(e) {
        var data = Rs (numberSelector).val();
        var cardType = determineCardType(getNumber(data));
        Rs (cardTypeSelector).text(cardType);
      });
    };
